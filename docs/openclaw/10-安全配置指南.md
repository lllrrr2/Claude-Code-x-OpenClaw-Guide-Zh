...
{
  sandbox: {
    provider: "docker",          // 使用 Docker 沙箱隔离
    // 沙箱限制 Agent 的文件访问、网络访问和命令执行
  }
}
```
...
openclaw approvals grant <tool-name>
openclaw approvals revoke <tool-name>
...
使用 Docker 沙箱将 Agent 的文件访问限制在容器内：
